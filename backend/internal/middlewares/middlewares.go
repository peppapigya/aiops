...
var (
	getRedisClient = database.GetRedisClient
	newRedisClient = redis.NewClient
	getBlockedKey  = func(redisClient *redis.RedisClient, key string) string {
		return redisClient.Get(key, false)
	}
	deleteBlockedKey = func(redisClient *redis.RedisClient, key string) error {
...
func redisOperator(claim *jwt.Claims, c *gin.Context, token string) {
	...
	if client == nil {
		log.Printf("redis 客户端未初始化，跳过单点登录冲突校验，user_id=%d", claim.GetUserId())
		return
	}
	...
	value := getBlockedKey(redisClient, key)
	if value != "" {
		_ = deleteBlockedKey(redisClient, key)
