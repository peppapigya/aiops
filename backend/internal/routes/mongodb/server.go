...
import (
	...
	"devops-console-backend/internal/model"
	mongoclient "devops-console-backend/internal/mongodb"
	"devops-console-backend/internal/service"
...
type Server struct {
	pool *mongoclient.Pool
}

func RegisterSubRouter(apiGroup *gin.RouterGroup) {
	server := &Server{pool: mongoclient.NewPool(mongoclient.DefaultConnectionConfig())}
	mongoGroup := apiGroup.Group("/mongodb")
	mongoGroup.GET("/openapi.json", func(c *gin.Context) {
		c.Data(http.StatusOK, "application/json; charset=utf-8", []byte(openAPISpec))
	})
	mongoGroup.GET("/health", server.health)
	mongoGroup.POST("/connection/test", server.testConnection)
	mongoGroup.GET("/meta/connection", server.connection)
	mongoGroup.GET("/databases", server.listDatabases)
	mongoGroup.POST("/databases", server.createDatabase)
	mongoGroup.DELETE("/databases/:db", server.deleteDatabase)
	mongoGroup.GET("/databases/:db/collections", server.listCollections)
	mongoGroup.POST("/databases/:db/collections", server.createCollection)
	mongoGroup.DELETE("/databases/:db/collections/:collection", server.deleteCollection)
	mongoGroup.GET("/databases/:db/collections/:collection/documents", server.listDocuments)
	mongoGroup.GET("/databases/:db/collections/:collection/documents/:id", server.getDocument)
	mongoGroup.POST("/databases/:db/collections/:collection/documents", server.createDocument)
	mongoGroup.PUT("/databases/:db/collections/:collection/documents/:id", server.updateDocument)
	mongoGroup.DELETE("/databases/:db/collections/:collection/documents/:id", server.deleteDocument)
	mongoGroup.POST("/databases/:db/collections/:collection/documents/bulk-delete", server.bulkDeleteDocuments)
	mongoGroup.GET("/databases/:db/collections/:collection/export", server.exportDocuments)
	mongoGroup.POST("/databases/:db/collections/:collection/import", server.importDocuments)
	mongoGroup.GET("/databases/:db/collections/:collection/backup", server.backupCollection)
	mongoGroup.POST("/databases/:db/collections/:collection/restore", server.restoreCollection)
	mongoGroup.GET("/databases/:db/collections/:collection/indexes", server.listIndexes)
	mongoGroup.POST("/databases/:db/collections/:collection/indexes", server.createIndex)
	mongoGroup.DELETE("/databases/:db/collections/:collection/indexes/:name", server.deleteIndex)
}
...
func (s *Server) testConnection(c *gin.Context) {
	...
	client, err := s.pool.Get(mongoclient.ConnectionConfig{
		Host:       request.Host,
		Port:       request.Port,
...
func (s *Server) connectionConfigFromRequest(c *gin.Context) mongoclient.ConnectionConfig {
	defaultConn := s.pool.DefaultConnection()

	return mongoclient.ConnectionConfig{
...
func (s *Server) clientForRequest(c *gin.Context) (*mongoclient.Client, bool) {
