...
func LoadConfig() error {
	...
	} else if err := expandConfigEnvVars(); err != nil {
		return err
	}
...
// expandConfigEnvVars 展开配置文件中的环境变量占位符
func expandConfigEnvVars() error {
	settings := viper.AllSettings()
	expanded := expandEnvValue(settings).(map[string]interface{})
	if err := viper.MergeConfigMap(expanded); err != nil {
		return fmt.Errorf("展开配置环境变量失败: %v", err)
	}
	return nil
}
...
func expandEnvValue(value interface{}) interface{} {
	...
	case map[string]interface{}:
		expanded := make(map[string]interface{}, len(typedValue))
		for key, item := range typedValue {
			expanded[key] = expandEnvValue(item)
		}
		return expanded
	case []interface{}:
		expanded := make([]interface{}, len(typedValue))
		for index, item := range typedValue {
			expanded[index] = expandEnvValue(item)
		}
		return expanded
	case string:
		return expandEnvString(typedValue)
	default:
		return typedValue
	}
}
