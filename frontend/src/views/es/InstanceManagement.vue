    ...
    <el-card v-if="viewMode === 'table'" class="content-card">
       ...
       >
          ...
          <el-table-column prop="name" label="名称" min-width="150" sortable="custom">
             <template #default="{ row }">
                <div class="flex items-center gap-2">
                   <el-icon :color="getTypeColor(getInstanceType(row))" size="18">
                      <component :is="getTypeIcon(getInstanceType(row))" />
                   </el-icon>
          ...
          <el-table-column prop="instance_type" label="类型" width="140" sortable="custom">
              <template #default="{ row }">
                 <el-tag :type="getTypeTagType(getInstanceType(row))" size="small" effect="plain">{{ getTypeLabel(getInstanceType(row)) }}</el-tag>
              </template>
          </el-table-column>
             ...
             <el-card class="content-card instance-card-view h-full" shadow="hover" :body-style="{ padding: '16px', height: '100%', display: 'flex', flexDirection: 'column' }">
                <div class="card-header flex justify-between items-start mb-4">
                   <div class="flex items-center gap-3">
                      <div class="icon-box" :style="{ backgroundColor: getTypeColor(getInstanceType(instance)) + '20', color: getTypeColor(getInstanceType(instance)) }">
                         <el-icon size="20"><component :is="getTypeIcon(getInstanceType(instance))" /></el-icon>
                      </div>
                      <div>
                         <div class="font-bold text-main">{{ instance.name }}</div>
                         <div class="text-xs text-sub">{{ getTypeLabel(getInstanceType(instance)) }}</div>
                      </div>
                   </div>
...
const fetchInstances = async () => {
  ...
  try {
    ...
    instances.value = (listData.data || []).map(instance => ({
      ...instance,
...
.page-container :deep(.el-pagination .el-input__wrapper) {
  background: var(--ds-bg-surface-2) !important;
  background-color: var(--ds-bg-surface-2) !important;
...
.page-container :deep(.el-select__selected-item),
.page-container :deep(.el-radio-button__inner),
.page-container :deep(.el-pagination__total),
.page-container :deep(.el-pagination__jump),
.page-container :deep(.el-pagination button),
...
.page-container :deep(.el-table__fixed-right),
.page-container :deep(.el-table__empty-block),
...
.page-container :deep(.el-table thead),
.page-container :deep(.el-table__header th) {
  background: var(--ds-bg-surface-2) !important;
  background-color: var(--ds-bg-surface-2) !important;
...
.page-container :deep(.el-table__body tr:hover > td.el-table__cell) {
  background: var(--ds-bg-hover) !important;
  background-color: var(--ds-bg-hover) !important;
}

.page-container :deep(.el-button:not(.is-link)) {
  background: var(--ds-bg-surface-2) !important;
  border-color: var(--ds-border-default) !important;
  color: var(--ds-text-secondary) !important;
  box-shadow: none !important;
...
.page-container :deep(.el-button--primary:not(.is-link)) {
  background: var(--ds-accent) !important;
  border-color: var(--ds-accent) !important;
  color: #fff !important;
}

.page-container :deep(.el-button--danger:not(.is-link)) {
  background: var(--ds-bg-danger-subtle) !important;
  border-color: rgba(239, 68, 68, 0.36) !important;
  color: var(--ds-error) !important;
}

.page-container :deep(.el-button.is-link) {
  background: transparent !important;
  border-color: transparent !important;
}

.page-container :deep(.el-alert) {
  background: var(--ds-bg-info-subtle) !important;
...
.border-t { border-top: 1px solid var(--ds-border-subtle); }
.text-main { color: var(--ds-text-primary); }
.text-sub { color: var(--ds-text-tertiary); }
...
.header-title-wrapper h2 { margin: 0; font-size: 18px; color: var(--ds-text-primary); }
.header-title-wrapper .subtitle { margin: 0; font-size: 12px; color: var(--ds-text-tertiary); }
