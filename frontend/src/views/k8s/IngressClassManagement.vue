  ...
  <div class="ingressclass-management">
    <el-card class="page-header-card cyber-card">
      <div class="page-header cyber-header">
        <div>
          <h2>路由类别管理</h2>
          <p>查看 Kubernetes IngressClass 资源</p>
        </div>
      </div>
    </el-card>
    ...
    <el-card class="content-card cyber-card">
      <div class="autoops-table-wrapper">
        ...
        >
          ...
          <el-table-column prop="controller" label="控制器" min-width="250" show-overflow-tooltip />
          ...
          <el-table-column prop="parameters" label="参数" width="200" show-overflow-tooltip>
            <template #default="scope">
    ...
    <el-dialog v-model="showDetailDialog" title="IngressClass 详情" width="800px">
      <div v-if="currentIngressClass">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="控制器">{{ currentIngressClass.spec.controller }}</el-descriptions-item>
        ...
        <el-divider content-position="left" v-if="currentIngressClass.spec.parameters">参数</el-divider>
        <el-descriptions v-if="currentIngressClass.spec.parameters" :column="1" border style="margin-top: 10px;">
          <el-descriptions-item label="类型">{{ currentIngressClass.spec.parameters.kind }}</el-descriptions-item>
          <el-descriptions-item label="名称">{{ currentIngressClass.spec.parameters.name }}</el-descriptions-item>
          <el-descriptions-item label="API组" v-if="currentIngressClass.spec.parameters.apiGroup">{{ currentIngressClass.spec.parameters.apiGroup }}</el-descriptions-item>
        </el-descriptions>
      </div>
    </el-dialog>
  </div>
