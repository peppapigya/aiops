<template>
  <div class="pod-page">
    <header class="pod-header">
      <div class="title-row">
        <h1>Pod管理</h1>
        <span>{{ podList.length }} 个</span>
      </div>
      <div class="header-actions">
        <el-select v-model="selectedNamespace" size="small" placeholder="Namespace" @change="handleNamespaceChange" class="namespace-select">
          <el-option label="全部" value="all" />
          <el-option v-for="ns in namespaceList" :key="ns.name" :label="ns.name" :value="ns.name" />
        </el-select>
        <el-button size="small" :loading="loading" @click="fetchPodList">
          <el-icon><Refresh /></el-icon>
          刷新
        </el-button>
        <el-button v-show="permStore.hasPerm('k8s:pod:showcreatedialogtrue')" size="small" type="primary" @click="showCreateDialog = true">
    ...
    <section class="pod-metrics">
      <div v-for="metric in podMetrics" :key="metric.key" class="metric-card" :class="`is-${metric.tone}`">
        <span>{{ metric.label }}</span>
        <strong>{{ metric.value }}</strong>
      </div>
    </section>

    <section class="pod-table-panel">
      ...
      <el-table
        ...
        :empty-text="loading ? '加载中' : '暂无Pod'"
      >
        <el-table-column prop="name" label="Name" min-width="220" show-overflow-tooltip>
          <template #default="scope">
            <div class="name-cell">
              <span class="status-dot" :class="statusTone(scope.row.status)"></span>
              <div>
        ...
        <el-table-column prop="ready" label="Ready" width="92">
          <template #default="scope">
            <span class="status-pill" :class="isReady(scope.row.ready) ? 'success' : 'warning'">{{ scope.row.ready || '0/0' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="Status" width="112">
          <template #default="scope">
            <span class="status-pill" :class="statusTone(scope.row.status)">{{ scope.row.status || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="restarts" label="Restarts" width="92">
          <template #default="scope">
            <span class="mono" :class="Number(scope.row.restarts || 0) > 0 ? 'warning' : 'success'">{{ scope.row.restarts || 0 }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="190" fixed="right" align="right">
          <template #default="scope">
            <el-button-group>
              <el-button v-show="permStore.hasPerm('k8s:pod:handleviewdetail')" size="small" @click="handleViewDetail(scope.row)"><el-icon><View /></el-icon></el-button>
              <el-button v-show="permStore.hasPerm('k8s:pod:handleopenterminal')" size="small" type="primary" @click="handleOpenTerminal(scope.row)"><el-icon><Monitor /></el-icon></el-button>
              <el-button v-show="permStore.hasPerm('k8s:pod:handledelete')" size="small" type="danger" plain @click="handleDelete(scope.row)"><el-icon><Delete /></el-icon></el-button>
            </el-button-group>
          </template>
        </el-table-column>
      </el-table>
                  ...
                  <el-col :span="12">
                    <el-form-item label="镜像拉取策略">
                      <el-select v-model="container.imagePullPolicy" placeholder="选择策略">
                        <el-option label="总是拉取" value="Always" />
                        <el-option label="本地不存在时拉取" value="IfNotPresent" />
                        <el-option label="从不拉取" value="Never" />
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="12">
    ...
    <el-dialog v-model="showDetailDialog" title="Pod 详情" width="1200px" :close-on-click-modal="false">
      <el-tabs v-model="detailTab" type="border-card">
        ...
        <el-tab-pane label="实时监控" name="monitoring">
          <div class="monitor-container" v-if="detailTab === 'monitoring'">
            <!-- 如果没有关联 Prometheus 实例，提示需要在配置里绑定，或者全局找一个 Prometheus 实例 proxy -->
    ...
    <!-- 新增/编辑自定义监控弹窗 -->
    <el-dialog v-model="showMonitorDialog" :title="monitorForm.id ? '编辑自定义图表' : '新增自定义图表'" width="600px" destroy-on-close>
      <el-form :model="monitorForm" :rules="monitorRules" ref="monitorFormRef" label-width="120px">
        <el-form-item label="图表标题" prop="title">
          <el-input v-model="monitorForm.title" placeholder="如：容器磁盘 I/O 速率" />
        </el-form-item>
        <el-form-item label="PromQL 模板" prop="promql_template">
          <el-input v-model="monitorForm.promql_template" type="textarea" :rows="3" placeholder="例如: rate(container_fs_io_time_seconds_total{pod='{{podName}}'}[5m])" />
          <div style="font-size: 12px; color: #999; margin-top: 5px;">
            可用插值变量: <code>{{podName}}</code>, <code>{{nodeName}}</code>
          </div>
        </el-form-item>
        ...
        <el-form-item label="Y轴单位">
          <el-input v-model="monitorForm.unit_suffix" placeholder="如: %, MB, 秒 (可选)" />
        </el-form-item>
...
const podMetrics = computed(() => {
  const total = podList.value.length
  const running = podList.value.filter((pod) => pod.status === 'Running').length
  const pending = podList.value.filter((pod) => ['Pending', 'ContainerCreating'].includes(pod.status)).length
  const failed = podList.value.filter((pod) => ['Failed', 'Error', 'CrashLoopBackOff'].includes(pod.status)).length
  const restarts = podList.value.reduce((sum, pod) => sum + Number(pod.restarts || 0), 0)
  return [
    { key: 'total', label: '总数', value: total, tone: 'info' },
    { key: 'running', label: '运行中', value: running, tone: 'success' },
    { key: 'pending', label: '等待中', value: pending, tone: 'warning' },
    { key: 'failed', label: '失败', value: failed, tone: 'error' },
    { key: 'restarts', label: '重启次数', value: restarts, tone: restarts > 0 ? 'warning' : 'success' }
  ]
})
const isReady = (ready) => {
...
const fetchCustomMonitors = async () => {
  try {
    const res = await getCustomMonitors('pod')
    ...
    console.error('Failed to fetch custom monitors:', error)
  }
}
...
.status-dot.success { background: var(--ds-success); }
.status-dot.warning { background: var(--ds-warning); }
.status-dot.error { background: var(--ds-error); }
.status-dot.neutral { background: var(--ds-text-muted); }
...
/* Dialog internals keep original function density, only align surface tokens */
.container-config,
.port-config,
.env-config,
.label-config,
