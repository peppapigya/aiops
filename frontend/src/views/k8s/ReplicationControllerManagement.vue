...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      this.loading = true
      try {
        const res = await getRCList(this.currentNamespace, this.instanceId)
        if (res.code === 200) {
          this.tableData = res.data.rcList
        }
      } catch (error) {
    ...
    async handleDetail(row) {
      try {
        const res = await getRCDetail(row.namespace, row.name, this.instanceId)
        if (res.code === 200) {
          this.detailData = JSON.stringify(res.data.rcDetail, null, 2)
    ...
    handleDelete(row) {
      this.$confirm(`确认删除 RC "${row.name}"?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const res = await deleteRC(row.namespace, row.name, this.instanceId)
          if (res.code === 200) {
...
.more-images-btn:hover {
  ...
  background: var(--ds-bg-hover, rgba(255,255,255,0.04));
}
