<template>
  <div class="mongodb-console" :class="{ 'mongodb-console--sidebar-collapsed': sidebarCollapsed }">
    <aside class="console-sidebar" :class="{ 'console-sidebar--collapsed': sidebarCollapsed }">
      ...
      <el-scrollbar v-if="!sidebarCollapsed" class="sidebar-scrollbar">
...
import {
  ...
  restoreCollection,
  updateDocument,
} from '../../api/mongodb/mongo'
import type { CollectionItem, ConnectionInfo, IndexItem } from '../../api/mongodb/types'
...
async function bootstrap() {
  if (!sessionConnection.value) {
    await router.replace({ name: 'MongoDBConnect', query: { redirect: '/mongodb' } })
    return
  }
...
async function goToConnect() {
  await router.push({ name: 'MongoDBConnect', query: { redirect: '/mongodb' } })
}
...
/* Match the rest of the DevOps console visual language. */
.mongodb-console {
  gap: 12px;
  padding: 12px;
...
.collection-button.active,
.collection-button:hover {
  background: var(--ds-bg-info-subtle, rgba(59,130,246,0.12));
  color: #409eff;
}
